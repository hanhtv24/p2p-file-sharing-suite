...
> **Đã đổi `PORT` mà vẫn báo `EADDRINUSE`?** Nguyên nhân thường gặp: dự án
> dùng `node --watch` (qua `npm run dev`) — khi server gặp lỗi, tiến trình
> **không tự thoát hẳn** mà chỉ in `Failed running... Waiting for file
> changes...` rồi treo đó, nghĩa là **tracker ở cổng 4000 của lần chạy trước
> vẫn còn sống**. Lần chạy sau dù đã đổi `PORT` cho server chính, biến
> `TRACKER_PORT` vẫn mặc định là 4000 → đụng độ với tiến trình cũ, vẫn báo
> `EADDRINUSE` (lúc này là do cổng 4000, không phải 5000 nữa). Cách xử lý dứt
> điểm — 2 bước:
> ```bash
> # 1) Tắt sạch mọi tiến trình node cũ
> killall node          # macOS/Linux
> taskkill /F /IM node.exe   # Windows (PowerShell/cmd)
>
> # 2) Chạy lại, đổi cả 2 cổng cho chắc
> PORT=5001 TRACKER_PORT=4001 npm run dev
> ```
> Nếu vẫn lỗi sau bước này, đọc kỹ dòng cuối của thông báo lỗi — nó luôn ghi
> rõ **số cổng nào** đang bị chiếm (`address: '::', port: <số>`), từ đó đổi
> đúng biến môi trường tương ứng (`PORT` cho server chính, `TRACKER_PORT` cho
> tracker).
