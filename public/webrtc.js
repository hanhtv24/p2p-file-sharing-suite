...
class PeerConnection {
  ...
  async addIceCandidate(candidate) {
    if (this.pc && candidate) {
      try {
        ...
        console.warn("[ICE] addIceCandidate error:", e.message);
      }
    }
  }
  ...
  async sendFile(file, fileId, onProgress) {
    ...
    for (let offset = 0; offset < buffer.byteLength; offset += CHUNK) {
      const slice = buffer.slice(
        offset,
        Math.min(offset + CHUNK, buffer.byteLength),
      );
      ...
      const corruptedHash =
        SIMULATE_CORRUPTION && chunkIndex === 1
      ...
      const frame = new ArrayBuffer(
        4 + 4 + fileIdB.length + 4 + hashB.length + slice.byteLength,
      );
      const view = new DataView(frame);
      const u8 = new Uint8Array(frame);
      let pos = 0;
      view.setInt32(pos, chunkIndex, true);
      pos += 4;
      view.setInt32(pos, fileIdB.length, true);
      pos += 4;
      u8.set(fileIdB, pos);
      pos += fileIdB.length;
      view.setInt32(pos, hashB.length, true);
      pos += 4;
      u8.set(hashB, pos);
      pos += hashB.length;
  ...
  close() {
    ...
    if (this.dataChannel) {
      try {
        ...
        console.warn("[close] dataChannel.close() lỗi:", e.message);
      }
    }
    if (this.pc) {
      try {
        ...
        console.warn("[close] pc.close() lỗi:", e.message);
      }
    }
  }
  ...
  _bindDataChannel(dc) {
    ...
    this.dataChannel.onerror = (err) =>
      console.error(`[DC ${this.remotePeerId}]`, err);
  ...
  _onJSON(msg) {
    ...
    switch (msg.type) {
      case "file-list":
        h._onFileList(this, msg.files);
        break;
      case "file-request":
        h._onFileRequest(this, msg.fileId);
        break;
      case "file-start":
  ...
  _onChunk(raw) {
    ...
    const chunkIndex = view.getInt32(pos, true);
    pos += 4;
    const fileIdLen = view.getInt32(pos, true);
    pos += 4;
    const fileId = new TextDecoder().decode(u8.slice(pos, pos + fileIdLen));
    pos += fileIdLen;
    const hashLen = view.getInt32(pos, true);
    pos += 4;
    const sentHash = new TextDecoder().decode(u8.slice(pos, pos + hashLen));
    pos += hashLen;
    ...
    sha256Hex(chunkData.buffer).then((computed) => {
      ...
      if (!ok)
        console.warn(`[Hash] Chunk ${chunkIndex} của "${rf.name}" bị lỗi!`);
...
class WebRTCHandler {
  ...
  _onDataChannelOpen(conn) {
    ...
    if (this.onPeerConnected)
      this.onPeerConnected(conn.remotePeerId, conn.remoteSocketId);
  }

  _onPeerICEConnected(conn) {
    // ICE connected — chờ data channel mở mới báo lên UI
    console.log(
      `[P2P ${conn.remotePeerId}] ICE connected, waiting for DataChannel...`,
    );
  }
  ...
  _onPeerDisconnected(conn) {
    ...
    if (this.onPeerDisconnected)
      this.onPeerDisconnected(conn.remotePeerId, conn.remoteSocketId);
  }

  _onFileList(conn, files) {
    if (this.onFileList)
      this.onFileList(conn.remotePeerId, conn.remoteSocketId, files);
  }
  ...
  _onFileRequest(conn, fileId) {
    ...
    // Bắt đầu gửi file
    conn
      .sendFile(shared.file, fileId, (fid, pct, cur, tot, idx, dir) => {
        if (this.onFileProgress) {
          this.onFileProgress(conn.remotePeerId, fid, {
            pct,
            cur,
            tot,
            chunkIdx: idx,
      ...
      .then(() => {
        if (this.onFileComplete) {
          this.onFileComplete(conn.remotePeerId, fileId, {
            blob: null,
            name: shared.name,
            size: shared.size,
            chunkStatus: [],
